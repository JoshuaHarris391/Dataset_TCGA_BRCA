...
complete_IHC_cases <- TCGA_BRCA_CLIN_MATCHED %>% select(c("CASE_ID", "ER_STATUS_BY_IHC", "PR_STATUS_BY_IHC", "IHC_HER2"))
...
# Adding annotations
complete_IHC_cases$SUBTYPE <- apply(complete_IHC_cases[, 2:4], 1, annotate_subtype)
complete_IHC_cases$SUBTYPE <- factor(complete_IHC_cases$SUBTYPE, levels = names(table(complete_IHC_cases$SUBTYPE)))

# Adding subtype annotation to TCGA_BRCA_CLIN_MATCHED
TCGA_BRCA_CLIN_MATCHED <- dplyr::left_join(TCGA_BRCA_CLIN_MATCHED, complete_IHC_cases[, c("CASE_ID", "SUBTYPE")], by = "CASE_ID")
